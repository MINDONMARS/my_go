...
/*
简单地来说，数据从一端流入，从另一端流出，看上去好像一个链表
...
func xrange() chan int { // 从开始自增的数生成器
...
func filter(in chan int, number int) chan int {
	//输入一个整数队列，筛出是number的倍数，不是number的倍数的放入输出队列
	//in: 输入队列
	out := make(chan int)
...
func main() {
	const max = 100  //找出100以内的素数
	nums := xrange() //初始化整数生成器
	number := <-nums // 从生成器抓取一个整数2，作为初始化整数
